				...
				struct ts_command *t_cmd, u16 len, void *val)
{
	...
	/* Read data */
	xfer[1].addr = t_cmd->addr;
	xfer[1].flags = I2C_M_RD;
	xfer[1].len = len + extend;
	xfer[1].buf = buf;
...
retry_read:
	...
	if (ret != ARRAY_SIZE(xfer)) {
		if (--retry) {
			dev_info(&client->dev, "%s: i2c retry reg %02x len %d\n", __func__,t_cmd->buf[0],len);
			msleep(TS_WAKEUP_TIME);
			goto retry_read;
		} else {
...
static int ts_check_retrigen(struct ts_data *data)
{
	...
	error = ts_read_reg(client,
					REG_PWM_OUTPUT_ENABLE, &val);
	if (error)
		return error;

	if (!val) {
		dev_warn(&client->dev, "Enabling PWM output\n");
		error = ts_write_reg(client,
						REG_PWM_OUTPUT_ENABLE, 1);
	}
...
static int ts_set_ts_power_cfg(struct ts_data *data, u8 sleep)
{
	...
	for(i = 0; i< 5; i++)	{
	 	error = ts_write_reg(data->client, REG_REPORT_ENABLE, enable);
	 	if (!error)
			 break;
		msleep(10);
	}
...
static int ts_read_info_block(struct ts_data *data)
{
	...
	dev_info(&client->dev, "chip memory %d(%02x)\n",val,val);
	if (val == 0 || val == 0xFF) 
		val = TS_REG_SIZE;
...
static int ts_load_fw(struct device *dev)
{
	...
	frame_buf = devm_kzalloc(dev,flash_size << 1, GFP_KERNEL);
	if (!frame_buf) {
	...
	while (pos < fw->size) {
		...
		hrec->offset = be16_to_cpu(hrec->offset);
		dev_dbg(dev, "get pos %d/%zd ofs 0x%04x count %d type %d\n",
			 pos, fw->size,hrec->offset,hrec->count,hrec->type);

		if (hrec->type == 0x0 &&
			hrec->offset + hrec->count < flash_size) {  //data frame
			memcpy(frame_buf + hrec->offset, hrec + 1, hrec->count);
		}else {
			dev_info(dev, "skip pos %d/%zd ofs 0x%04x count %d type %d\n",
				 pos, fw->size,hrec->offset,hrec->count,hrec->type);
		}
	...
	dev_info(dev, "Sent %d frames, %zd bytes success\n", frame, ofs);

#if defined(CONFIG_TS_UPDATE_CHECK)
	for (ofs = 0, frame = 0; ofs < flash_size; ofs += TS_MAX_BLOCK_WRITE) {
		/* Read one frame to device */
		ret = ts_bootloader_read(data->client, TS_BOOT_RW, ofs, frame_buf + flash_size + ofs,TS_MAX_BLOCK_WRITE);
		if (ret)
			goto disable_irq;

		frame++;
		if (frame % 10 == 0) {
			dev_info(dev, "Read %d frames, %d/%zd bytes\n",
				 frame, ofs, flash_size);
		}
	}

	dev_info(dev, "Read %d frames, %zd bytes success\n", frame, ofs);

	ret = memcmp(frame_buf,frame_buf + flash_size,flash_size);
	if (ret) {
		dev_err(dev, "compare fw failed %d\n",ret);
		goto disable_irq;
	}

	dev_info(dev, "compare %d frames, %zd bytes success\n", frame, ofs);
#endif
...
int ts_check_firmware_version(struct ts_data *data, const char *version_str)
{
	struct device *dev = &data->client->dev;
	...
	dev_info(dev, "[ts] firmware version %s - %s\n",version_str, firmware_version);
					...
					const char *buf, size_t count)
{
	...
	error = ts_check_firmware_version(data,data->fw_name);
	if (error)
		return error;
...
#	endif
	...
	dev_info(&client->dev, "Ts probe finished\n");
